...
public abstract class FileTransfer {
    ...
    private List<FileReceiveProcessListener> fileReceiveProcessListeners;
    ...
    protected void writeToStream(final InputStream in, final OutputStream out)
        ...
        do {
            ...
            try {
                fireProcess(amountWritten, fileSize);
            } catch (Exception e) {
            }
    ...
    private void initFileReceiveProcessListeners() {
        fileReceiveProcessListeners = new ArrayList<FileReceiveProcessListener>();
    }

    public void addFileReceiveProcessListener(FileReceiveProcessListener fileReceiveProcessListener) {
        if (fileReceiveProcessListeners == null) {
            initFileReceiveProcessListeners();
        }
        synchronized (this.fileReceiveProcessListeners) {
            fileReceiveProcessListeners.add(fileReceiveProcessListener);
        }
    }

    public void fireProcess(Long receivedByte, Long fileSize) {
        if (fileReceiveProcessListeners != null) {
            FileReceiveProcessListener[] listeners = null;
            synchronized (this.fileReceiveProcessListeners) {
                listeners = new FileReceiveProcessListener[this.fileReceiveProcessListeners.size()];
                this.fileReceiveProcessListeners.toArray(listeners);
            }
            for (FileReceiveProcessListener fileReceiveProcessListener : listeners) {
                fileReceiveProcessListener.processReceived(receivedByte, fileSize);
            }
        }
    }
